...
const API_BASE_URL = "http://10.0.2.2:8000/api/auth"; // Android Emulator
...
export const sendOTPAPI = async ({ mobile }) => {
  try {
    console.log("[AUTH SERVICE] Sending OTP to:", mobile);
    ...
    console.log("[AUTH SERVICE] OTP mock sent successfully to:", mobile);
...
/**
 * Complete user signup/profile (Host)
 * @param {Object} data - { mobile, fullName, email, password }
 * @returns {Promise<{token: string, user: Object}>}
 */
export const signupAPI = async ({ mobile, fullName, email, password }) => {
  try {
    console.log("[AUTH SERVICE] Signup host attempt:", { email, mobile });
